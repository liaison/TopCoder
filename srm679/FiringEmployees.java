...
Each employee can be described by three numbers:
    their salary, their productivity, and the number of their direct manager.

For each employee E, employee E's manager has a number smaller than E.
Hence, the company has a tree-like hierarchy.

You are given the description of BottomCoder in the int[]s manager, salary, and productivity.

For each i between 1 and N, inclusive,
    employee i is described by the values manager[i-1], salary[i-1], and productivity[i-1].
...
As another example, if he fires the manager of your manager,
he must fire your manager and therefore he must also fire you.

Find a valid way of firing some (possibly none, possibly all)
employees that maximizes the profit of the resulting company.
...
NOTES
-The CEO of BottomCoder has both salary and productivity equal to zero.
The CEO cannot be fired.
...
However, if he does that, he would have to fire all four employees,
    because 1 is (either directly or indirectly) the manager of each of them.
...
public class FiringEmployees {
	...
	public int fire(int[] manager, int[] salary, int[] productivity) {
	    ...
	    for(int i=0; i<manager.length; ++i) {
	        ...
	        if(profit < 0) {
	            int cascadeProfit = profitToFire(i);

	            if(cascadeProfit > 0) {
	                // still keep this manager.
	                sum += profit;
	            }
	        } else {
	            sum += profit;
	...
	protected int profitToFire(int employer) {

	    int profit = productivity[employer] - salary[employer];

	    // Fire all the people that the employer manages.
	    for (int i=0; i<manager.length; ++i) {

	        // We cannot fire the CEO.
	        if(manager[i] == 0)
	            continue;

	        if(manager[i] == employer) {
	            profit += profitToFire(i);
	        }
	    }
	...
	public static void main(String[] args) {
		FiringEmployees temp = new FiringEmployees();
        ...
        */
		...
		System.out.println(temp.fire(manager, salary, productivity));
	}
